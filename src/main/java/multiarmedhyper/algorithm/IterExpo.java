...
public class IterExpo extends ExpoHyper {
    
    String name = "IterExpo";
    
    public IterExpo(ParameterReader param, double eta) {
        super(param, eta);
    }
    
    public IterExpo(ParameterReader param) {
        super(param);
    }
    ...
    public void solve() {
        ...
        while (app < iterLimit) {
            ...
            if (apply == 1) {
                ...
                next = (new Random()).nextInt(nLowLevel);
            }
    ...
    boolean acceptSolution(double tmpCost) {
        double rho = !best.isFeasible() ? 1E-3 
                : 1E-5 + delta * (1.0 - app * 1.0 / iterLimit);
        return (tmpCost <  (1 + rho) * bestValue);
    }
    
    @Override
    public String algorithmName() {
        return name + "[" + eta + "," + delta + "]";
    }
}
